...
if __name__ == "__main__":
    ...
    gs.update(left=0.08, right=0.98, bottom = 0.06, top=0.98, hspace = 0.12,
               wspace=0.085)
    ...
    ylims = [[9.6,10.3], [-2.4,1.2], [-0.5,0.75], [-3.5, 1.5]]
    ...
    for i, (y, y1, y2) in enumerate(zip(data, errs1, errs2)):
        ...
        ax.scatter(r, y, c=sn, s=45, cmap=cmap, zorder=2,
                   lw=0.1, norm=norm)
        ######################################################################
        ...
        ax.errorbar(r_l12, l12[i], yerr = [l12_errs1[i], l12_errs2[i]],
                    color="r", ecolor=lgray,
                        ...
                        alpha=1, ms=7, mew=0.2)
        # ax.yaxis.set_major_locator(plt.MaxNLocator(5))
        ax.set_xlim(-1.5, 0.8)
        ######################################################################
        # Plot data for Coccato+ 2011
        ax.plot(rlodo, data_lodo[i], "^y", mec="k", ms=7, mew=0.1)
        #####################################################################
        ...
        ax2.errorbar(mu, y, yerr = [y1, y2], fmt=None,
                    color=lgray, ecolor=lgray, capsize=0., zorder=1)
        ax2.scatter(mu, y, c=sn, s=45, cmap=cmap, zorder=2,
                   lw=0.1, norm=norm)
        ax2.set_xlim(19,25)
        ######################################################################
        # Plot data for Loubser 2012
        ...
        ######################################################################
        # Plot data for Coccato+ 2011
        ax2.plot(mulodo, data_lodo[i], "^y", mec="k", ms=7, mew=0.1)
        #####################################################################
        if i == 3:
